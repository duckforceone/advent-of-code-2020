...
//bro this is literally a fucking assembly interpreter in fucking JS
const fs = require('fs');

const task = fs.readFileSync('input.asm', {encoding: 'utf-8'});
...
const p = new AssemblyInterpreter(task, { infiniteLoopWarning: true })
p.run();
...
for (let i = 0; i < code.length; i++) {
    ...
    if(element.instruction === 'nop' || element.instruction === 'jmp') {
        ...
        const fixedProgram = new AssemblyInterpreter(newSource);
        fixedProgram.run();
