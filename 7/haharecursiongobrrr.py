...
with open("input.txt") as reader:
    for l in reader.readlines():
        ...
        COLORS[color] = [bags.strip() for bags in re.sub(r" bags|bag\.*", "", contains).split(", ")]
...
def det_num_bag(color_dict: dict, root_color: str, target_color: str = None) -> int:
    ...
    if isinstance(bags := color_dict[root_color], int):  # if the amount of target_color bags was already determined, just return it
        return bags
